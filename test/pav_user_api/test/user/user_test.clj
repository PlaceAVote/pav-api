...
(against-background [(before :facts (do (bootstrap-constraints)
 ...
 (facts "Test cases for users"
   ...
   (fact "Create a new user, will return 201 status and newly created user"
         (let [response (app (content-type (request :put "/user" (ch/generate-string {:email "john@stuff.com" :password "stuff2" :country-code 840})) "application/json"))]
           (:status response) => 201
           (keys (ch/parse-string (:body response) true)) => (contains [:token :email :country-code] :in-any-order)))

  (fact "Create a new user, with an existing email, should return 409"
        (let [_ (app (content-type (request :put "/user" (ch/generate-string {:email "john@stuff.com" :password "stuff2" :country-code 840})) "application/json"))
              response (app (content-type (request :put "/user" (ch/generate-string {:email "john@stuff.com" :password "stuff2" :country-code 840})) "application/json"))]
          (:status response) => 409
  ...
  (fact "Create a new user, when the payload is missing an email, return 400 with appropriate error message"
        (let [response (app (content-type (request :put "/user" (ch/generate-string {:password "stuff2" :country-code 840})) "application/json"))]
          (:status response) => 400
  ...
  (fact "Create a new user, when the payload is missing a password, return 400 with appropriate error message"
        (let [response (app (content-type (request :put "/user" (ch/generate-string {:email "john@stuff.com" :country-code 840})) "application/json"))]
          (:status response) => 400
  ...
  (fact "Create a new user, when the email is invalid, return 400 with appropriate error message"
    (let [response (app (content-type (request :put "/user" (ch/generate-string {:email "johnstuffcom" :password "stuff2" :country-code 840})) "application/json"))]
      (:status response) => 400
  ...
  (fact "Create a new user, when the country is invalid, return 400 with appropriate error message"
        (let [response (app (content-type (request :put "/user" (ch/generate-string {:email "john@stuff.com" :password "stuff2" :country-code ""})) "application/json"))]
          (:status response) => 400
  ...
  (fact "Create a new user, when the password is invalid, return 400 with appropriate error message"
      (let [response (app (content-type (request :put "/user" (ch/generate-string {:email "john@stuff.com" :password "" :country-code 840})) "application/json"))]
        (:status response) => 400
  ...
  (fact "Retrieve a user by email"
         (let [{:keys [token]} (ch/parse-string (:body (app (content-type (request :put "/user" (ch/generate-string {:email "john@stuff.com" :password "stuff2" :country-code 840})) "application/json"))) true)
