...
(against-background [(before :contents (do
  ...
  (facts "Test cases covering retrieval of user profiles and following and unfollowing users"
    ...
    (fact "Retrieve a users profile in relation to current user"
      (let [{caller-token :token} (:body (pav-reqv2 :put "/user" (new-pav-user)))
            {following :user_id} (:body (pav-reqv2 :put "/user" (new-pav-user)))
            _ (pav-reqv2 :put (str "/user/follow") caller-token {:user_id following})
            {status :status body :body} (pav-reqv2 :get (str "/user/" following "/profile") caller-token {})]
        status => 200
    ...
    (fact "Retrieve a users profile without an authentication token, Then verify user is returned."
      (let [{caller-token :token} (:body (pav-reqv2 :put "/user" (new-pav-user)))
            {following :user_id} (:body (pav-reqv2 :put "/user" (new-pav-user)))
            _ (pav-reqv2 :put (str "/user/follow") caller-token {:user_id following})
            {status :status body :body} (pav-reqv2 :get (str "/user/" following "/profile"))]
        status => 200
        (set (keys body)) => (set [:user_id :first_name :last_name :country_code :state :public :total_followers :total_following :following])
    ...
    (fact "Retrieve a user profile, when the user profile doesn't exist, Then return 404 error"
      (let [{caller :body} (pav-reqv2 :put "/user" (new-pav-user))
            {status :status response :body} (pav-reqv2 :get (str "/user/21312321312/profile") (:token caller) {})]
        status => 404
        response => {:error {:error_message "User Profile does not exist"}}))

    (fact "Retrieve the current users profile"
      (let [{user :body} (pav-reqv2 :put "/user" (new-pav-user))
            {status :status body :body} (pav-reqv2 :get "/user/me/profile" (:token user) {})]
        status => 200
    ...
    (fact "Retrieve the current users profile when the users origin is facebook"
      (let [{caller :body} (pav-reqv2 :put "/user/facebook" (new-fb-user))
            {status :status body :body} (pav-reqv2 :get "/user/me/profile" (:token caller) {})]
        status => 200
    ...
    (fact "Follow/following another user"
      (let [{follower :body} (pav-reqv2 :put "/user" (new-pav-user {:first_name "john" :last_name "stuff"}))
            {my_id :user_id token :token} follower
            {being-followed :body} (pav-reqv2 :put "/user" (new-pav-user {:first_name "paul" :last_name "the blow"}))
            {pauls_user_id :user_id} being-followed
            {create_status :status} (pav-reqv2 :put (str "/user/follow") token {:user_id pauls_user_id})
            {my-following :body} (pav-reqv2 :get (str "/user/me/following") token {})
            {paul-following :body} (pav-reqv2 :get (str "/user/" pauls_user_id "/following") token {})
            {my-followers :body} (pav-reqv2 :get (str "/user/me/followers") token {})
            {pauls-followers :body} (pav-reqv2 :get (str "/user/" pauls_user_id "/followers") token {})]
        create_status => 201
    ...
    (fact "Follow a user, When that user is the same person, Then return 400 error."
      (let [{body :body} (pav-reqv2 :put "/user" (new-pav-user))
            ...
            {status :status} (pav-reqv2 :put (str "/user/follow") token {:user_id user_id})]
        status => 400))

    (fact "Unfollow user"
      (let [{follower :body} (pav-reqv2 :put "/user" (new-pav-user))
            {token :token} follower
            {being-followed :body} (pav-reqv2 :put "/user" (new-pav-user))
            {pauls_user_id :user_id} being-followed
            _ (pav-reqv2 :put (str "/user/follow") token {:user_id pauls_user_id})
            {unfollow-status :status} (pav-reqv2 :delete (str "/user/unfollow") token {:user_id pauls_user_id})
            {my-following :body} (pav-reqv2 :get (str "/user/me/following") token {})
            {pauls-followers :body} (pav-reqv2 :get (str "/user/" pauls_user_id "/followers") token {})]
        unfollow-status => 204
        my-following => []
    ...
    (fact "Given a user with no followers, When the user follows another user, Then publish event to users timeline."
      (let [{follower :body} (pav-reqv2 :put "/user" (new-pav-user))
            ...
            _ (pav-reqv2 :put (str "/user/follow") follower-token {:user_id followed_user_id})
            _ (Thread/sleep 2000)
            {status :status body :body} (pav-reqv2 :get "/user/me/timeline" follower-token {})
