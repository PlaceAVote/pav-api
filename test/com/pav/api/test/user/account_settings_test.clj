...
(against-background [(before :contents (do
	...
	(fact "Retrieve a users account settings"
		(let [user (new-pav-user)
          ...
          {status :status body :body} (pav-reqv2 :get "/user/me/settings" token {})]
			status => 200
			body =>
	...
	(fact "Retrieve a facebook users account settings"
		(let [user (new-fb-user)
          {body :body} (pav-reqv2 :put "/user/facebook" user)
          {token :token user_id :user_id} body
          {status :status body :body} (pav-reqv2 :get "/user/me/settings" token {})]
			status => 200
	...
	(fact "Update a users account settings"
		(let [user (new-pav-user)
          ...
          _ (pav-reqv2 :post "/user/me/settings" token changes)
					{status :status body :body} (pav-reqv2 :get "/user/me/settings" token {})]
			status => 200
    ...
    (let [user (new-pav-user)
          ...
          changes {:public false :first_name "Ted" :last_name "Baker" :gender "f" :dob "06/10/1986"
                   :email "Johnny5@placeavote.com" :city "New York City"}
          {status :status body :body} (pav-reqv2 :post "/user/me/settings" token changes)]
      status => 400
	...
	(fact "Try updating a users account settings, when given an invalid field.  Throw 400 error."
        (let [{body :body} (pav-reqv2 :put "/user" (new-pav-user))
              {token :token} body
              {status :status body :body} (pav-reqv2 :post "/user/me/settings" token {:invalid "invalidfield here"})]
          status => 400
  ...
  (fact "Try updating user account with an invalid email, Throw 400 error"
		(let [{body :body} (pav-reqv2 :put "/user" (new-pav-user))
          {token :token} body
          {status :status} (pav-reqv2 :post "/user/me/settings" token {:email "john.com"})]
          status => 400))
	...
	(fact "Try updating users email address.  Ensure you can login with new email address"
		(let [{:keys [password] :as user} (new-pav-user)
          {body :body} (pav-reqv2 :put "/user" user)
          {token :token} body
          _ (pav-reqv2 :post "/user/me/settings" token {:email "newemail@placeavote.com"})
          {status :status} (pav-reqv2 :post "/user/authenticate" {:email "newemail@placeavote.com" :password password})]
      status => 201))
	...
	(fact "Try updating user with empty payload. Ensure user remains untouched"
		(let [user (new-pav-user)
          {body :body} (pav-reqv2 :put "/user" user)
					{token :token user_id :user_id} body
					_ (pav-reqv2 :post "/user/me/settings" token {})
					{status :status body :body} (pav-reqv2 :get "/user/me/settings" token {})]
			status => 200
			body =>
			(merge {:user_id user_id :public true :social_login false}
