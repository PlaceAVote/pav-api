...
(deftest db-functions
  (fact "Test parsing database url for different databases."
    ...
    (d/parse-url "h2:file:///tmp/pav") => {:subprotocol "h2" :subname "file:///tmp/pav;mode=mysql"}
    (d/parse-url "h2:mem:pav") => {:subprotocol "h2" :subname "mem:pav;mode=mysql;db_close_delay=-1"})
