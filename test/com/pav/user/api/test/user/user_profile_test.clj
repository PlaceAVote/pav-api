...
(def searchable-profile {:email "peter@pl.com" :password "stuff2" :first_name "peter" :last_name "pan" :dob "10/05/1984"
...
(against-background [(before :facts (do
	...
	(fact "Retrieve a users profile in relation to current user"
		(let [{caller :body} (pav-req :put "/user" test-user)
					...
					{status :status body :body} (update-in (pav-req :get (str "/user/" user_id "/profile") token {}) [:body] #(ch/parse-string % true))]
			status => 200
      (set (keys body)) => (set [:user_id :first_name :last_name :img_url :country_code :state :public :total_followers :total_following :following])
	...
	(fact "Retrieve a users profile without an authentication token, Then verify user is returned."
		(let [{caller :body} (pav-req :put "/user" test-user)
					...
					{user_id :user_id} (ch/parse-string search-user true)
					_ (pav-req :put (str "/user/follow") token {:user_id user_id})
					{status :status body :body} (update-in (pav-req :get (str "/user/" user_id "/profile")) [:body] #(ch/parse-string % true))]
			status => 200
      (set (keys body)) => (set [:user_id :first_name :last_name :img_url :country_code :state :public :total_followers :total_following :following])
  ...
  (fact "Retrieve a user profile, when the user profile doesn't exist, Then return 401 error"
	...
	(fact "Retrieve the current users profile"
		(let [{caller :body} (pav-req :put "/user" test-user)
			...
			status => 200
      (keys body) => (contains [:user_id :first_name :last_name :img_url :country_code :state :public :total_followers :total_following] :in-any-order)
