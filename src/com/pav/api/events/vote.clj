...
(s/defn ^:always-validate create-timeline-vote-event :- VoteTimelineEvent
  ...
  ([event_id {:keys [bill_id vote-id user_id timestamp]}]
    (VoteTimelineEvent. event_id "vote" bill_id vote-id user_id timestamp))
  ([event]
...
(s/defn ^:always-validate create-newsfeed-vote-event :- VoteNewsFeedEvent
  "Accepts a user vote, and returns an event suitable for publishing to the users notifications."
  ([event_id {:keys [bill_id vote-id user_id timestamp]}]
    (VoteNewsFeedEvent. event_id "vote" bill_id vote-id user_id timestamp false))
  ([event]
    (create-newsfeed-vote-event (.toString (UUID/randomUUID)) event)))